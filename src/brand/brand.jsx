  ...
  const handleRemove = (brand) => {
    ...
    deleteBrand(brand._id)
      .then((data) => {
        ...
        dispatch(
          setBrandsSuccess(
            brandInfo.data.filter((item) => item._id !== brand._id)
          )
        );
        setState((prev) => ({
          ...prev,
      ...
      .catch((e) => {
        message.error(
          getHttpErrorMessage(e, `Error while deleting ${brand.title}`)
        );
  ...
  return (
    ...
    >
      <Table
        columns={[
          {
            ...
            render: (url, record) => (
              <div>
                <Avatar src={urlHelper.fileUrl(record.img)} /> {record.title}
              </div>
            ),
          },
          {
        ...
        loading={brandInfo.loading}
        dataSource={brandInfo.data}
      />
    </Card>
  );
}
